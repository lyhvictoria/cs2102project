...
-- Get info of restaurant selected via restaurantId
-- $1 = restaurantId
...
-- Get info of restaurant selected via restaurant name
-- $1 = restaurant name
SELECT restaurantId, area, name, minSpendingAmt
FROM Restaurants
WHERE name = $1
...
-- $1 = restaurant name
SELECT DISTINCT M.itemName, M.price, M.category, M.isAvailable
...
-- View all their past reviews made by customer
-- $1 = customerId
SELECT DISTINCT O.orderDate, R.name, RV.review, RV.rating
FROM Reviews RV JOIN Orders O USING (orderId)
    JOIN OrderDetails OD USING (orderId)
...
-- Create a order
-- $1 = customerId, $2 = orderDate
-- $3 = orderTime, $4 = mode of payment, either 'Card' or 'Cash'
...
-- $1 = orderId, $2 = restaurantId
-- $3 = itemName, $4 = quantity
...
-- $1 = delivery location, $2 = area of location, $3 = OrderId
...
-- $1 = orderId, $2 = time left to restaurant
UPDATE Orders
...
-- $1 = orderId, $2 = time reached at Restaurant
UPDATE Orders
...
-- $1 = orderId, $2 = time left to Destination
UPDATE Orders
...
-- $1 = orderId, $2 = time reached at Destination
UPDATE Orders
...
-- Use rewards points
-- $1 = orderId, $2 = points used
UPDATE Customers
SET rewardPoints = rewardPoints - $2
WHERE customerId = $1
;

-- Customer select payment mode
-- $1 = orderId, $2 = mode of payment, either 'Card' or 'Cash'
UPDATE Orders
SET paymentMode = $2
WHERE orderId = $1
;
...
-- View menu items for their restaurant, sorted via $2
-- $2 = to sort by price/amount left/category
...
-- Add a new menu item
-- $1 = restaurantId, $2 = itemName, $3 = price
-- $4 = category, $5 = amtLeft
...
-- $1 = restStaffId, $2 = specific year, $3 = specific month
WITH temp AS (
    SELECT DISTINCT EXTRACT(YEAR FROM (O.orderDate)) AS year, EXTRACT(MONTH FROM (O.orderDate)) as month, O.orderId, O.totalCost
    FROM Orders O JOIN OrderDetails OD USING (orderId)
    WHERE OD.restaurantID = (
	     SELECT restaurantId
...
SELECT DISTINCT D.promotionId,
		CASE
		    ...
		    ELSE totalOrders
		    END AS totalOrders
		, durationInDays, durationInHours,
    CASE
		    WHEN OM.totalOrders IS NULL THEN 0
...
-- $1 = riderId of the delivery rider
with totalWorkedHours as (
...
--View total salary earned, hours worked, orders delivered by each rider for each month
-- $1 = riderId of delivery rider
With computePT as (
...
FROM (
	...
	FROM FullTime FT INNER JOIN DeliveryRiders DR using (riderId)
	INNER JOIN WorkingWeeks WW using (riderId)
...
from v1_1 one natural join v1_2 two left join v1_3 three using (riderId, year, month)
where one.riderId = $1
order by one.year, one.month
;

--View hours worked by each Part-time rider for each week
-- $1 = riderId of delivery rider
Select riderId, year, month, week, sum(intervalTime) weeklyHoursWorked
From (
  Select riderId, extract(year from workDate) as year, extract(month from workDate) as month, extract(week from workDate) as week, intervalStart, intervalEnd, (intervalEnd - intervalStart) as intervalTime
  From WorkingDays
  Where riderId = $1
) as Foo
Group By riderId, year, month, week
...
Insert into WorkingWeeks ($1, $2, $3, $4);
...
-- For each month
-- view total number of new Customers, total orders and total costs of these orders
with v4_1 as (
...
-- $1 = restaurantId
SELECT OD.itemName, SUM(OD.quantity) as quantitypurchased
...
SELECT OD.itemName, SUM(OD.quantity) as quantitypurchased, OD.restaurantId
FROM OrderDetails OD
GROUP BY OD.restaurantId, OD.itemName
Order By SUM(OD.quantity) desc, OD.itemName
;
